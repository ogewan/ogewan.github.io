...
import { fireColophonEasterEgg } from './colophon-easter-egg';
...
// CelestialBackdrop. The browser-console easter egg lives in
// ./colophon-easter-egg.ts; it carries its own once-per-load guard.
export function ColophonSection() {
  const { t } = useTranslation(['colophon', 'common']);

  useEffect(() => {
    fireColophonEasterEgg();
  }, []);
...
│   ├── celestial/            : persistent backdrop, R3F scenes
│   ├── ng-elements/          : Angular custom-element timeline
│   ├── manifest-builder/     : Zod schema + Octokit CLI, run by GH Actions
│   └── content/              : i18n strings + locales/{en,es}
├── manifest.json             : generated by manifest-builder
└── .github/workflows/        : build-and-deploy + dispatch-receiver`}
          </pre>
        </GlassPanel>
      </Section>
      ...
      >
        <GlassPanel className="p-6">
          <table className="w-full font-mono text-small">
            ...
            <tbody>
              <ChangeRow ver="1.3" date="2026-05-30" note={t('sections.changelog.entries.v12')} />
              <ChangeRow ver="1.2" date="2026-05-25" note={t('sections.changelog.entries.v11')} />
